...
class UserDataSource @Inject constructor(
    ...
    //zakomentirano jer ne radi, previše upita u isto vrijeme
    /*fun noOfAllReports(userId: Int): Flow<APIResult<Map<String, Int>>> {
        return flow {
            emit(APIResult.Loading)
            try {
                val reports = mutableMapOf<String, Int>()
                val projectList = mutableListOf<Project>()
                val projects = supabaseClient.postgrest["Project"].select {
                    filter("owner", FilterOperator.EQ, userId)
                }.decodeList<Project>()
                for (pr in projects) {
                    projectList.add(pr)
                ...
                for (project in projectList) {
                    if (!reports.contains(project.name)) {
                        val res = supabaseClient.postgrest["Report"].select {
                            filter("id_project", FilterOperator.EQ, project.id_project)
                        }.decodeList<Report>()
                        //reports[project.name] = res.count()
                        //reports.put(project.name, res.count())
                        var newPair: Map<String, Int> = mapOf(project.name to res.count())
                        Log.e("projekt", project.name)
                        Log.e("novi par", newPair.toString())

                        reports.put(project.name, res.count())
                        Log.e("izvjestaji", reports.toString())
                    }
                }
                emit(APIResult.Success(reports))
            } catch (e: Exception) {
